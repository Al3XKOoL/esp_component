...
void CustomClimate::loop() {
    ...
    if (tiempo_actual - ultimo_tiempo_verificacion_ >= intervalo_segundos_ * 1000) {
        ...
        float temp_sol = sensor_temp_sol_ ? sensor_temp_sol_->state : NAN;
        float temp_agua = sensor_temp_agua_ ? sensor_temp_agua_->state : NAN;
        float temp_salida = sensor_temp_salida_ ? sensor_temp_salida_->state : NAN;
        ...
        if (this->mode == esphome::climate::CLIMATE_MODE_HEAT) {
            ...
            bool estado_bomba_actual = interruptor_bomba_ ? interruptor_bomba_->state : false;
            ...
            if (interruptor_bomba_ != nullptr && interruptor_bomba_->state) {
                interruptor_bomba_->turn_off();
