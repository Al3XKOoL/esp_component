...
void CustomClimate::loop() {
  ...
  if (current_time - last_check_time_ >= interval_seconds_ * 1000) {
    ...
    esph_log_i(TAG, "Ejecutando loop()");
    ...
    if (espera_) {
      if (tiempo_actual >= tiempo_espera_fin_) {
        espera_ = false;
        esph_log_i(TAG, "Reanudando verificaciones después de espera de 5 minutos.");
      } else {
        esph_log_i(TAG, "En espera hasta %lld", tiempo_espera_fin_);
        return;
      }
    }
    ...
    if (temp_sun_->state > (temp_water_->state + diff_high_) && temp_water_->state < temp_max_) {
      if (!estado_bomba_actual) {
        ...
        esph_log_i(TAG, "Bomba encendida debido a la temperatura adecuada");
      } else {
        esph_log_i(TAG, "Bomba ya está encendida");
      }
    } else {
      if (estado_bomba_actual) {
        ...
        esph_log_i(TAG, "Bomba apagada debido a temperatura inadecuada");
        esph_log_w(TAG, "Tiempo total de funcionamiento de la bomba: %lld segundos", tiempo_total_encendido);
        espera_ = true;
        tiempo_espera_fin_ = tiempo_actual + 300; // 5 minutos en segundos
        return;
      } else {
        esph_log_i(TAG, "Bomba ya está apagada");
        return;
      }
    }
    ...
    if (estado_bomba_actual && temp_output_->state < (temp_water_->state + 1)) {
      ...
      int64_t tiempo_total_encendido = tiempo_actual - tiempo_inicio_;
      tiempo_encendida_ += tiempo_total_encendido;
      esph_log_i(TAG, "Bomba apagada debido a temperatura de salida insuficiente");
      esph_log_w(TAG, "Tiempo total de funcionamiento de la bomba: %lld segundos", tiempo_total_encendido);
      espera_ = true;
      tiempo_espera_fin_ = tiempo_actual + 300; // 5 minutos en segundos
      return;
    ...
    if (estado_bomba_actual) {
      ...
      esph_log_w(TAG, "Tiempo transcurrido de funcionamiento de la bomba: %02d:%02d:%02d",
               (int)(tiempo_transcurrido / 3600),
               (int)((tiempo_transcurrido % 3600) / 60),
               (int)(tiempo_transcurrido % 60));
    }

    esph_log_w(TAG, "Diferencia Sol-Agua: %.2f°C", temp_sun_->state - temp_water_->state);
    esph_log_w(TAG, "Diferencia Salida-Agua: %.2f°C", temp_output_->state - temp_water_->state);
    esph_log_w(TAG, "Estado de la bomba: %d", pump_switch_->state);
    esph_log_i(TAG, "Conteo de encendidos de la bomba: %d", conteo_encendidos_);
