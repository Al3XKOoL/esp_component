    ...
    if (current_time - last_check_time_ >= interval_seconds_ * 1000) {
        ...
        if (espera_) {
            if (tiempo_actual >= tiempo_espera_fin_) {
                espera_ = false;
                ESP_LOGI(TAG, "Reanudando verificaciones después de espera de 5 minutos.");
            } else {
                ESP_LOGI(TAG, "En espera hasta %lld", tiempo_espera_fin_);
                return;
            }
        }
        ...
        ESP_LOGI(TAG, "Conteo de encendidos de la bomba: %d", conteo_encendidos_);
